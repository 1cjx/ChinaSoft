...
bool OLED_remote_call(char *str){
    if(not_ready) return false;
    OLED_ShowString(x_num, ((y_num++)*12), str, 12);
    return true;
}
bool OLED_update(){
    updata_flag = true;
...
/******************************************************************************/ 
{
    ...
    while(1)
    {
        ...
        x_num = QR_size*QR_scale+QR_scale;
        // app上线地址
        const char QRMap[] = 
"\
#######_#_##__#___#######\
...
#######_#___#_####_#_####";
        for(int i=0; i<QR_size; i++){
            for(int j=0; j<QR_size; j++){
                if(QRMap[i*QR_size+j] == '#'){
                    for(int ii=0; ii<QR_scale; ii++){
                        for(int jj=0; jj<QR_scale; jj++){
                            OLED_DrawPoint(QR_scale*j+jj, QR_scale*i+ii, 1);
                        }
                    }
                }
            }
