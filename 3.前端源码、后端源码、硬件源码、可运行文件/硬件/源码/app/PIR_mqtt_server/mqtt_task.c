...
int mqtt_connect(void)
{
	...
	sprintf(buff, "IoT_%d_%d_%d_rip", deviceId, facilityId, facilityType);
	...
	// /* subscribe 订阅主题 */
	sprintf(buff + 16, "L/%d", facilityId);
	...
	while (1)
	{
		while (MQTTPacket_read(buf, buflen, transport_getdata) == PUBLISH)
		{
			...
			// MQTTSerialize_publish   构造PUBLISH报文的函数
			// MQTTDeserialize_publish 解析PUBLISH报文的函数
			...
			int lamp_level = atoi(payload_in);	// 获取灯的亮度
			lamp_set(lamp_level);	// 设置灯的亮度
		}
